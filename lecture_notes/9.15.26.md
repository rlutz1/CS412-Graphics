...
+ and then indices need to be set in that triangular pattern--defines the triangle vertices that actually need to be connected (otherwise, what connections do we even use)
...
+ ELEMENT_ARRAY_BUFFER -> likely for integer types that are not floats
  + indices that point to vertices in array_buffer?
